...
function TopicBadge({ category }: { category: GrammarTopic["category"] }) {
  return (
    <span className="inline-flex items-center rounded-full bg-stone-100 dark:bg-stone-800 px-2 py-0.5 text-xs text-stone-600 dark:text-stone-300">
  ...
  return (
    <li>
      <Link
        ...
        className="flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-stone-100 dark:hover:bg-stone-800 focus:outline-none focus:ring-2 focus:ring-green-600 transition-colors"
      >
...
export default function KielioppiPage() {
  ...
  return (
    <div className="flex flex-1 flex-col min-h-0">
      <header className="border-b border-stone-200 dark:border-stone-800 px-4 py-4 space-y-3">
                    ...
                    <ul className="space-y-1">
                      {topics.map((topic) => (
                        <li key={topic.slug}>
                          <Link
                            href={`/kielioppi/${topic.slug}`}
                            className="block rounded-lg px-2 py-1.5 text-sm text-stone-700 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-800 hover:text-stone-900 dark:hover:text-stone-100 focus:outline-none focus:ring-2 focus:ring-green-600 transition-colors"
                          >
                            {topic.title}
                          </Link>
                        </li>
